...
到 ../trade-ts/baseline/golden/*.json。TS 侧逐条断言,浮点用 1e-9 相对容差。
...
OUT = ROOT.parent / "trade-ts" / "baseline" / "golden"
...
def gen_prompts():
    entries = []
    for version in sorted(p.name.split("_v")[1][:-3] for p in (ROOT / "prompts").glob("system_v*.md")):
        v = "v" + version
...
def gen_shorthand():
    ...
    cases = [
        ...
        # v2 守卫:中心偏离现价 >20%(写漏标的)/两个行权价量级的数/带理由
