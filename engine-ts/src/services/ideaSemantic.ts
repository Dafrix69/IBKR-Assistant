 ...
 *  · 分数 ≥ MIN_SCORE、且在前 TOP_K 名的才算「语义相近」——qwen3-embedding 0.6B 在这批笔记上分数挤在 0.25–0.62,
 *    相关的 0.55 以上,不相关的大多 0.48 以下(2026-09-23 实测),门槛宁高勿低;
...
export class IdeaSemanticService extends ServiceBase {
  /** 余弦低于这个不算相近(见文件头的实测) */
  static readonly MIN_SCORE = 0.5;
  /** 最多补几条 */
  static readonly TOP_K = 20;
  /** 一次最多给多少条想法算向量 */
  ...
  async rank(query: string, pool: readonly Idea[]): Promise<Map<string, number> | null> {
    ...
    try {
      ...
      const scored = pool
        ...
        .filter(([, s]) => s >= IdeaSemanticService.MIN_SCORE)
        .sort((a, b) => b[1] - a[1])
...
function defaultEmbedder(): Embedder | null {
  ...
  try {
    return new OllamaEmbedder(process.env["DAFRI_EMBED_URL"] || DEFAULT_EMBED_URL, process.env["DAFRI_EMBED_MODEL"] || DEFAULT_EMBED_MODEL);
  } catch {
