 ...
 * **只给 golden-rpc 钉着的字段用**:基线里有一条请求没带它,期望的却是 handler 的某句原话——要么就是它自己那句
 * (ideas.update 不带 id →「缺少想法 id」),要么是排在它前面的另一项检查(backtest.run 只给一个坏代码 →「股票代码不合法」,
 * 这时 start / end / strategy 都没带)。换成 schema 的「缺少 xx」就是改了基线。
