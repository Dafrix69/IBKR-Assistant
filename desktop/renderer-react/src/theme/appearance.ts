...
// ---- 涨跌配色 --------------------------------------------------------------
const updownListeners = new Set<Listener>();
let updown: UpDown = read('dafri-updown') === 'red-up' ? 'red-up' : 'green-up';

export function applyUpDown(mode: UpDown): void {
  updown = mode === 'red-up' ? 'red-up' : 'green-up';
  document.documentElement.dataset.updown = updown;
  write('dafri-updown', updown);
  updownListeners.forEach((l) => l());
...
export function useUpDown(): UpDown {
  return useSyncExternalStore(
    (l) => {
      updownListeners.add(l);
      return () => {
        updownListeners.delete(l);
      };
    },
    () => updown,
  );
...
export function initAppearance(): void {
  ...
  void dafri.setTheme(themeMode).catch(() => {});
