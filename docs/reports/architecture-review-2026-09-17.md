...
### 2026-09-21,真机只读核对:15 / 15

TWS 起来之后跑了 `npm run probe`(只读:临时配置 / 临时库、三道发单闸门强制关、client id 71 起)。
**这是拆过的 `engine.ts`、迁完的 77 个契约方法第一次在真机上走。**

- 过的:持仓 5 条(含一条合成的看跌价差组合行)、AAPL / SPY 盘口含深度、SPX 5m 与 AAPL 1d 历史 K 线、
  SPX 期权链 21 档带希腊值、宏观行情带、成交同步、盯盘循环在跑(43 轮)、
  夜盘 SPX 按 ES 202612 减基差推算(源标成 `futures` 并带出推算说明)。
- **修了探针自己的一个 bug**:`entitlements` 那一步硬取 `connections[0]`,而这次只开了 7496(live)、
  7497(paper)没开,于是它对着一个没开的端口等满 10 秒报「底层连接超时」——那不是行情权限的问题,
  是探针挑错了连接。改成**用 `broker.connect` 真正连上的那条**。改完这一步报
  **六个品种(正股 / 期货 / 指数 / 期权)全部实时**。
- 探针报出的那条诊断**其实是引擎的误报,已修**:`别名 模拟 对不上这个 TWS 可管的账户,发到它们的单会被拒`。
  「模拟」是 DU 开头的纸面账户、配在 `paper`(7497)上,而这条诊断拿它去对 `live`(7496)能管的账户。
  `checkAliasMapping` 不分 `connection`,把所有别名都往当前连接上核——**等于让用户去修一个没坏的配置**。
  改成:给了连接名就只核那条连接上的别名(不给的老调用方照旧全核,所以既有断言一条都没动);
  `unmapped`(会话能管到、而别名表里完全没有的账号)仍对着全部配置算——那是真信号,和挂哪条连接无关。
  富途那条路同一个毛病一起改了(它原来靠 `rows[i]` 和 `settings.accounts` 下标对齐,改成按别名回查)。
  补了三条测试(live 只出主账户 / paper 只出模拟 / 不给连接名仍全核)。真机复核:
  `别名 主账户✓ · 未配别名 ["U1***311"]`,误报没了、真信号还在。
- **仍然没做的**:模拟账户里从真界面建追踪 / 切启停 / 立即平仓。7497 没开、`模拟` 别名也对不上,
  这一步做不了;而且会发单的操作不由助手代做。合并前还差这一步。
...
### 回头修正报告自己的一条判断:体积预算需要工具(2026-09-21)

上面「机制:让它保持住」第 3 条写的是:**"体积预算。不需要工具,写在 CLAUDE.md 里就够。"**
**这条判断错了,而且有数据**:从那之后到 2026-09-20,`engine.ts` 长到 2,527 行、`broker.ts` 2,509 行,
界面五个页面 400–875 行。规矩写在文件里,拦不住它长——写规矩的人和写代码的人是同一个,
而写代码的时候不会回头读规矩。

09-20/21 花了十二刀把它们拉回来(九个页面全进 400,`engine.ts` −29%,合约工具从 `broker.ts` 分出去)。
这些成果没有闸门就会再长回去,所以补了 `engine-ts/tests/size-budget.spec.ts`,照 `LEGACY_METHODS` 的
同一种写法:

- 已超线的钉在 `OVER_BUDGET` 里(现在 5 个:`broker.ts` 2400、`engine.ts` 1790、`futuBroker.ts` 1740、
  `lib/chart/engine.ts` 535、`lib/chart/overlays.ts` 483),**只许变短、数字只许变小**;
- 不在名单里的文件新超线 → 红,报出"先问一句:它是不是两个东西";
- 名单里的文件降到预算内 → 也红,逼你把它划掉(名单不许留陈货);
- 单独一条钉住**所有 `pages/` 都在 400 以内**——那是九刀拆出来的成果,值得单独守。

写完**验过它真的会拦**:给 `engine.ts` 加 5 行 → 「名单里的文件只许变短」红;
给刚进预算的 `Market.tsx` 加 400 行 → 「没有新的文件超线」和「所有页面组件都在预算内」一起红。
验完还原,全量 1,200 个用例绿。
